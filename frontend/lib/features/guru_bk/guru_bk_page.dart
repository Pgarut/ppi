  ...
  Widget _currentPage() {
    if (_activeFeature != null) {
      switch (_activeFeature) {
        case 'pengaduan': return const PengaduanPageBK();
        case 'konseling': return const KonselingPage();
        case 'bakat-minat': return const BakatMinatPage();
        case 'monitoring': return const MonitoringAkademikPage();
        case 'laporan': return const LaporanPageBK();
        default: return DashboardPageBK(onFeatureTap: _openFeature, onLogout: _logout);
      }
    }
    switch (_shellTab) {
        ...
        NavigationRail(
          ...
          leading: Padding(
            ...
            child: Row(children: [
              ...
              const Text('BK', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
            ]),
          ),
          trailing: Padding(
            padding: const EdgeInsets.all(8),
