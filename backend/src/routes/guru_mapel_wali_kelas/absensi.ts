...
export async function handleAbsensiGuru(request: Request, env: Env, user: UserPayload, pathParts: string[], url: URL): Promise<Response> {
  ...
  if (subPath === 'absensi/assignments' && request.method === 'GET') {
    let assignments = await env.DB.prepare(`
      SELECT DISTINCT gmp.mata_pelajaran_id, mp.nama as mapel_nama, mp.kode as mapel_kode,
    ...
    if (assignments.results.length === 0) {
      assignments = await env.DB.prepare(`
        SELECT DISTINCT gm.mata_pelajaran_id, mp.nama as mapel_nama, mp.kode as mapel_kode,
