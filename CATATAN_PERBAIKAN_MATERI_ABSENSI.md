...
- **Materi:** ditampilkan **preview PDF di dalam halaman** (viewer `pdfx`), termasuk dukungan otomatis untuk link sharing Google Drive.
...
> ```powershell
> flutter pub add youtube_player_flutter pdfx
> ```
...
Halaman ini mengunduh PDF (termasuk konversi otomatis dari link sharing Google Drive → direct download), lalu menampilkannya dengan `pdfx`.
...
- Kedua package (`youtube_player_flutter`, `pdfx`) **kompatibel web + mobile**, jadi aman untuk deploy web (Cloudflare Pages) maupun mobile.
